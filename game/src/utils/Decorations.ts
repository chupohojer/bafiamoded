...
type UsernameDecorationOptions = {
  ...
  /*
    Android profile username decorations are not always just
    "text width + padding". Some animation contexts keep a wider
    pill around short nicknames. This option lets each screen
    reproduce that without changing Ratings/Dashboard globally.
  */
  animationMinWidth?: string;
...
export function renderUsernameDecorations(
  ...
  */
  ...
  /*
    Original Android keeps UsernameAnimation on a neutral translucent
    rounded plate even when the player has no separate UsernameBackground.
    A real UsernameBackground from the server always has priority.
  */
  if(!usernameBackground?.["1"]) {
    wrapper.style.background =
  ...
  if(options.animationMinWidth) {
    wrapper.style.minWidth =
      options.animationMinWidth;
  }
