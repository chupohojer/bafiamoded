...
if(usernameAnimationMarketItem) {
  ...
  /*
    Main preview must reproduce the real nickname-decoration geometry.
    renderUsernameDecorations() below owns padding/background/Lottie layer;
    this wrapper only limits it to the card width.
  */
  ...
  function renderMainPreview() {
    ...
    /*
      Feed the currently selected market values into the SAME renderer that
      Dashboard/Profile use after activation. This makes the shop preview
      represent the real pill width instead of a tiny shop-only box.

      Short nicknames keep the accepted Android-like minimum width:
        mobile  135px
        desktop 152px
      Longer names can still grow naturally up to the card width.
    */
