...
function recolorLottie(data: any, androidColor?: string) {
    ...
    const cssColor = androidColorToCss(androidColor);

    if(!/^#[0-9a-fA-F]{6,8}$/.test(cssColor)) {
        return result;
    }
    ...
    function replaceColorProperty(property: any) {
        if(!property || !Array.isArray(property.k)) {
            return;
        }

        // Обычный статический цвет
        ...
        // Анимированный цвет
        property.k.forEach((frame: any) => {
            if(Array.isArray(frame?.s) && frame.s.length >= 3) {
                frame.s[0] = r;
                frame.s[1] = g;
                frame.s[2] = b;
            }

            if(Array.isArray(frame?.e) && frame.e.length >= 3) {
                frame.e[0] = r;
                frame.e[1] = g;
                frame.e[2] = b;
            }
    ...
    function replaceGradient(gradient: any) {
        if(!gradient?.k || !gradient?.p) {
            return;
        }

        const points = Number(gradient.p);

        function replaceValues(values: any[]) {
            if(!Array.isArray(values)) {
                return;
            }

            // Каждый цветовой stop:
            // position, R, G, B
        ...
        if(Array.isArray(property.k)) {
            if(typeof property.k[0] === 'number') {
                replaceValues(property.k);
            }
            else {
                property.k.forEach((frame: any) => {
                    if(Array.isArray(frame?.s)) {
                        replaceValues(frame.s);
                    }

                    if(Array.isArray(frame?.e)) {
                        replaceValues(frame.e);
                    }
                });
            }
        }
...
if(node.ty === 1 && typeof node.sc === 'string') {
  ...
  selectedPeriod = value;
    ...
    /*
      Long leaderboard avatar loading:
      only visible/nearby rows are requested.
      The real image is assigned the instant it finishes loading.
    */
    const avatarLoaders =
      new WeakMap<
        HTMLImageElement,
        ...
        ? new IntersectionObserver(
            entries => {
              for(const entry of entries) {
                ...
                const load =
                  avatarLoaders.get(img);

                avatarLoaders.delete(img);
                load?.();
              }
            },
            {
  ...
  ratingList.innerHTML = '';
        ...
        users.forEach((user, index) => {
          ...
          playerRow.style.gridTemplateColumns = '58px 1fr 45px';
...
const avatarDecorations = user.dcrs ?? {};
const photoBorder = avatarDecorations['8'];

if(photoBorder?.['1']) {
  avatar.style.border =
    `3px solid ${androidColorToCss(photoBorder['1'])}`;
  ...
  avatar.style.boxSizing = 'border-box';
          ...
          /*
            Show a local avatar immediately. The real photo replaces it
            in-place as soon as that request completes — no reload needed.
          */
          getDefaultAvatar()
            .then(src => {
              if(!avatar.src)
          ...
          playerInfo.style.paddingLeft = '2px';
...
playerNameRow.style.alignItems = 'center';
playerNameRow.style.gap = '8px';
playerNameRow.style.maxWidth = '100%';

playerInfo.appendChild(playerNameRow);
          ...
          const decorations = user.dcrs ?? {};

/*
  1 = UsernameBackground
  2 = UsernameShadow
  3 = UsernameText
*/

const usernameBackground = decorations['1'];
const usernameShadow = decorations['2'];
const usernameText = decorations['3'];
...
if(usernameText?.['1']) {
  playerName.style.color =
    androidColorToCss(usernameText['1']);
}
...
if(usernameShadow?.['1']) {
  const shadowColor =
    androidColorToCss(usernameShadow['1']);
...
playerNameWrap.style.position = 'relative';
playerNameWrap.style.display = 'inline-flex';
playerNameWrap.style.alignItems = 'center';
playerNameWrap.style.width = 'fit-content';
playerNameWrap.style.flexShrink = '0';
playerNameWrap.style.overflow = 'hidden';
playerNameWrap.style.borderRadius = '10px';
playerNameWrap.style.boxSizing = 'border-box';

playerNameRow.appendChild(playerNameWrap);
playerNameWrap.appendChild(playerName);
if(usernameBackground?.['1']) {
    playerNameWrap.style.background =
        androidColorToCss(usernameBackground['1']);

    playerNameWrap.style.padding = '2px 8px';
    playerNameWrap.style.borderRadius = '8px';
}
          const usernameAnimation = user.dcrs?.['0'];

if(usernameAnimation?.['2']) {

   
    /*
   
      Same Android-like animation plate as shared Decorations.ts.
   
      Explicit UsernameBackground keeps its server color.
    ...
    */
    ...
    playerNameWrap.style.padding = '2px 16px';
    playerNameWrap.style.minHeight = '32px';

    const animationLayer = document.createElement('div');
    animationLayer.style.position = 'absolute';
    animationLayer.style.left = '0';
    animationLayer.style.right = '0';
    animationLayer.style.top = '0';
    animationLayer.style.bottom = '0';
    ...
    animationLayer.style.zIndex = '0';

    playerName.style.position = 'relative';
    playerName.style.zIndex = '1';

    playerNameWrap.insertBefore(animationLayer, playerName);
    playerName.style.position = 'relative';
    ...
    const animationData =
    usernameAnimations[usernameAnimation?.['2']];
...
const animation = lottie.loadAnimation({
    ...
    const speed =
        Number(usernameAnimation['3'] ?? 100) / 100;

    const alpha =
        Number(usernameAnimation['0'] ?? 100) / 100;


        animation.setSpeed(speed);
    animationLayer.style.opacity = `${alpha}`;
    const animationColor =
    androidColorToCss(usernameAnimation['1']);
...
if(animationColor) {
    animation.addEventListener('DOMLoaded', () => {
        ...
        const filter =
            document.createElementNS(ns, 'filter');

        const filterId =
            `usernameTint_${index}_${Math.random()
                .toString(36)
                .slice(2)}`;
        ...
        const flood =
            document.createElementNS(ns, 'feFlood');
...
if(vipVariant) {
  ...
  vipVariant === '1'
    ? '👑'
    : vipVariant;
  vip.style.fontSize = '22px';
  vip.style.flexShrink = '0';

  playerNameRow.appendChild(vip);
}

          const playerValue = document.createElement('div');

          const ratingLabel =
    selectedMetric === 'Соревновательный'
        ? 'Очки рейтинга'
        : selectedMetric;

playerValue.textContent =
    `${ratingLabel}: ${user[PacketDataKeys.RATING_VALUE] ?? 0}`;
